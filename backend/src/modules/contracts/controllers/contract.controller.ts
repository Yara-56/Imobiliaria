...
export const createContract = async (req: Request, res: Response) => {
  try {
    const { propertyId, renterId, rentAmount, dueDay, startDate, endDate, depositValue, paymentMethod, contractNumber, notes } = req.body;
    ...
    return res.status(201).json({ status: "success", message: "Contrato criado!", data: { contract } });
  } catch (error) {
    console.error(error);
...
export const updateContractStatus = async (req: Request, res: Response) => {
  try {
    ...
    if (updated.count === 0) return res.status(404).json({ message: "Contrato não encontrado." });
...
export const deleteContract = async (req: Request, res: Response) => {
  try {
    ...
    if (deleted.count === 0) return res.status(404).json({ message: "Contrato não encontrado." });
